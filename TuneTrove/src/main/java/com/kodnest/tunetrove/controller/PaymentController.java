...
import com.kodnest.tunetrove.service.UserService;
...
public class PaymentController {
	...
	public String createOrder(HttpSession session) {
		...
		Order order=null;
		try {
			RazorpayClient razorpay=new RazorpayClient("rzp_test_DH0cERGu4XUj3Q", "HEjyEhY0D37b0MlUrhUVeQFw");
	...
	public boolean verifyPayment(@RequestParam  String orderId, @RequestParam String paymentId,
		...
		try {
			...
			RazorpayClient razorpayClient = new RazorpayClient("rzp_test_DH0cERGu4XUj3Q", "HEjyEhY0D37b0MlUrhUVeQFw");
			// Create a signature verification data string
			String verificationData = orderId + "|" + paymentId;

			// Use Razorpay's utility function to verify the signature
			boolean isValidSignature = Utils.verifySignature(verificationData, signature, 
					"HEjyEhY0D37b0MlUrhUVeQFw");
